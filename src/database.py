...
CREATE TABLE IF NOT EXISTS bom_verses (
    ...
    text TEXT NOT NULL,
    word_count INTEGER,
    biblical_source TEXT,             -- "Isaiah 14:1" if this verse quotes the Bible
...
-- Flexible tagging system for matches
CREATE TABLE IF NOT EXISTS match_tags (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    match_id INTEGER NOT NULL,
    tag TEXT NOT NULL,
    source TEXT DEFAULT 'auto',           -- 'auto', 'llm', 'manual'
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (match_id) REFERENCES matches(id) ON DELETE CASCADE,
    UNIQUE(match_id, tag)
);

CREATE INDEX IF NOT EXISTS idx_match_tags_match ON match_tags(match_id);
CREATE INDEX IF NOT EXISTS idx_match_tags_tag ON match_tags(tag);
"""

# Official tags we use programmatically
OFFICIAL_TAGS = {
    # Biblical source flags
    'isaiah_quote': 'BOM verse is quoting Isaiah',
    'nt_quote': 'BOM verse is quoting New Testament',
    'ot_quote': 'BOM verse is quoting Old Testament (non-Isaiah)',
    'biblical_parallel': 'Match is to a biblical passage',

    # Match types (can also be in match_type column)
    'direct_copy': 'Near-verbatim textual borrowing',
    'paraphrase': 'Same idea expressed differently',
    'thematic': 'Shared theme or concept',
    'stylistic': 'Similar writing style/patterns',
    'structural': 'Similar narrative structure',
    'name_borrow': 'Borrowed proper nouns',

    # Thematic categories
    'war_narrative': 'Battle/military narrative parallels',
    'restoration_prophecy': 'Prophecies about restoration of Israel',
    'hebrew_origin': 'Native Americans as Hebrews theme',
    'great_spirit': 'Great Spirit / deity concept',
    'buried_record': 'Hidden/buried sacred records theme',

    # Provenance flags
    'ces_letter_known': 'Documented in CES Letter',
    'wordtree_known': 'Found in WordTree 4-gram study',
    'novel_discovery': 'Potentially new finding',

    # Review flags
    'high_confidence': 'High confidence match',
    'needs_review': 'Requires manual review',
    'false_positive': 'Marked as not a real parallel',
}
...
class Database:
    ...
    def add_tag(self, match_id: int, tag: str, source: str = 'auto') -> None:
        """Add a tag to a match."""
        with self.get_connection() as conn:
            conn.execute(
                """INSERT OR IGNORE INTO match_tags (match_id, tag, source)
                   VALUES (?, ?, ?)""",
                (match_id, tag, source)
            )

    def add_tags_batch(self, tags: List[Dict]) -> int:
        """Batch add tags. Each dict should have match_id, tag, and optionally source."""
        with self.get_connection() as conn:
            for t in tags:
                if 'source' not in t:
                    t['source'] = 'auto'
            conn.executemany(
                """INSERT OR IGNORE INTO match_tags (match_id, tag, source)
                   VALUES (:match_id, :tag, :source)""",
                tags
            )
            return len(tags)

    def remove_tag(self, match_id: int, tag: str) -> None:
        """Remove a tag from a match."""
        with self.get_connection() as conn:
            conn.execute(
                "DELETE FROM match_tags WHERE match_id = ? AND tag = ?",
                (match_id, tag)
            )

    def get_tags_for_match(self, match_id: int) -> List[str]:
        """Get all tags for a match."""
        with self.get_connection() as conn:
            rows = conn.execute(
                "SELECT tag FROM match_tags WHERE match_id = ?",
                (match_id,)
            ).fetchall()
            return [row['tag'] for row in rows]

    def get_matches_by_tag(self, tag: str, limit: int = 100) -> List[Dict]:
        ...
        with self.get_connection() as conn:
            rows = conn.execute(
                """SELECT m.*, bv.text as verse_text, bv.book, bv.chapter, bv.verse,
                          sp.text as source_text, sp.source_name, sp.location
                   FROM matches m
                   JOIN match_tags mt ON m.id = mt.match_id
                   JOIN bom_verses bv ON m.bom_verse_id = bv.id
                   JOIN source_passages sp ON m.source_passage_id = sp.id
                   WHERE mt.tag = ?
                   ORDER BY m.final_score DESC
    ...
    def get_matches_without_tag(self, tag: str, limit: int = 100) -> List[Dict]:
        """Get matches that do NOT have a specific tag."""
        with self.get_connection() as conn:
            rows = conn.execute(
                """SELECT m.*, bv.text as verse_text, bv.book, bv.chapter, bv.verse,
                          sp.text as source_text, sp.source_name, sp.location
                   ...
                   JOIN source_passages sp ON m.source_passage_id = sp.id
                   WHERE m.id NOT IN (
                       SELECT match_id FROM match_tags WHERE tag = ?
                   )
                   ORDER BY m.final_score DESC
                   LIMIT ?""",
                (tag, limit)
    ...
    def get_tag_counts(self) -> Dict[str, int]:
        """Get count of matches for each tag."""
        with self.get_connection() as conn:
            rows = conn.execute(
                "SELECT tag, COUNT(*) as count FROM match_tags GROUP BY tag ORDER BY count DESC"
            ).fetchall()
    ...
    def insert_matches_batch(self, matches: List[Dict]) -> int:
        """Batch insert matches. Returns count inserted."""
        with self.get_connection() as conn:
            conn.executemany(
                """INSERT OR REPLACE INTO matches
                   (bom_verse_id, source_passage_id, embedding_score, final_score,
                    match_type, confidence, detection_methods)
                   VALUES (:bom_verse_id, :source_passage_id, :embedding_score, :final_score,
                           :match_type, :confidence, :detection_methods)""",
                matches
            )
            return len(matches)
